            ...
            for c_x in range(mesh_chunks):
                ...
                for y in range(c_y*chunk_size,((c_y+1)*chunk_size)-edge_y):
                    for x in range(c_x*chunk_size,((c_x+1)*chunk_size)-edge_x):
                        ...
                        tris1.addVertex(index3)
                        tris1.addVertex(index2)
                        tris1.addVertex(index1)
                        tris1.addVertex(index2)
                        tris1.addVertex(index3)
                        tris1.addVertex(index4)

                        tris2.addVertex(index1)
                        tris2.addVertex(index2)
                        tris2.addVertex(index3)
                        tris2.addVertex(index4)
                        tris2.addVertex(index3)
                        tris2.addVertex(index2)

                tris1.closePrimitive()
                f_geoms[(c_y*mesh_chunks)+c_x].addPrimitive(tris1)
                tris2.closePrimitive()
                b_geoms[(c_y*mesh_chunks)+c_x].addPrimitive(tris2)
        print("100.0%")
