"use strict";
// Transcrypt'ed from Python, 2017-02-07 10:22:25
//edited by Samuel Simpson
//DO NOT attempt to change this as the server has verification to check that clients are valid
function chat_client_compact () {
   var __symbols__ = ['__py3.6__', '__esv5__'];
    ...
    function init() {
        var cookie = getCookie("ChatWebClientNickname");
        if (cookie != null) {
          //cookienick = (String(cookie).split(","))[0];
          cookienick = cookie;
          if (cookienick != "") {
            var cookie = getCookie("ChatWebClientID");
            if (cookie != null) {
              //id = (String(cookie).split(","))[0];
              id = cookie;
              if (id != "") {
                gotid = true;
              }
            ...
            CWLog("Please enter a nickname. Go to homepage, enter a nickname and press Join. If this problem persists please ensure your cookies are enabled.");
          }
        }
        else {
          CWLog("Please enter a nickname. Go to homepage, enter a nickname and press Join. If this problem persists please ensure your cookies are enabled.");
    ...
    function CWLog(stl) {
      stl = stl + "<br>";
      document.getElementById ('frag_1').innerHTML = document.getElementById ('frag_1').innerHTML + stl;
      var objDiv = document.getElementById("frag_1");
      objDiv.scrollTop = objDiv.scrollHeight;

    }

    function testWebSocket() {
       websocket = new WebSocket(hostserv);

       websocket.onopen = function(evt) {
          print("connected to socket");
          connected = true;
          connecting = false;
          setTimeout(LoadCW(), 500);
       };
       ...
       websocket.onmessage = function(evt) {
         ...
         reader.onload = function(){
           ...
           if (!gotid) {
             id = indata;
             gotid = true;
             var name = String("ChatWebClientID"), value = String(id), path = 'path', domain = 'domain', ATS_getExpire = function() { return 'ATS_getExpire'; };
             ...
             var output = indata.replace("\n","<br>");
             document.getElementById ('frag_1').innerHTML = document.getElementById ('frag_1').innerHTML + output;
             var objDiv = document.getElementById("frag_1");
             objDiv.scrollTop = objDiv.scrollHeight;
           }
         };
       ...
       websocket.onclose = function(evt) {
          if (connected) {
            console.log("SOCKET CLOSED: " + evt.data);
            CWLog("Disconnected from server.");
          }
       };
    }

    function LoadCW() {
      chat_client_compact.ChatWindow.__init__()
    }

    window.addEventListener("load", LoadCW, false);

    function sendonws(dts) {
      websocket.send(String(dts));
    }
	...
	(function () {
		var ChatWindow = __class__ ('ChatWindow', [object], {
			...
			get toMD5 () {return __get__ (this, function (strtohash) {
				var MD5 = new Hashes.MD5().hex(strtohash);
				return String(MD5);
			});},
			get longcallback () {return __get__ (this, function () {
				var msg = document.getElementById ('text_1').value;
        ...
        var smsg = false;
          ...
          if (c.charCodeAt(charindex) == 10) {
            smsg = true;
				...
				document.getElementById ('text_1').value = outtext;
        if (smsg) {
          chat_client_compact.ChatWindow.sendnewmessage();
        }
			});},
        ...
        chat_client_compact.ChatWindow.longcallback();
				var msgdata = document.getElementById ('text_1').value;
				if (msgdata.__getslice__ (0, 6, 1) == '~admin') {
					try {
						var msgdatatmp = msgdata.py_split (' -', 2);
						msgdata = (('~admin -' + msgdatatmp [1]) + ' -') + chat_client_compact.ChatWindow.toMD5(msgdatatmp [2]);
					}
					catch (__except0__) {
						msgdata = '~admin';
					}
					...
					try {
						...
						msgdata = (msgdatatmp [0] + ' -') + chat_client_compact.ChatWindow.toMD5(msgdatatmp [1]);
					}
					catch (__except0__) {
						msgdata = msgdatatmp [0];
					}
				}
        if (msgdata.length > 0) {
				  sendonws (msgdata);
          ...
          if (connected) {
  					...
  					var firsttime = true;
  					try {
  						sendonws ('cf7bcef89b9cf428535a77d5bdc972c8');
  						sendonws (cookienick);
              sendonws (String(gotid));
              if (gotid) {
    						sendonws (id);
    						sendonws ('WebClient');
    						var message = 'Connected to remote host. You can start sending messages.';
...
window ['chat_client_compact'] = chat_client_compact ();
