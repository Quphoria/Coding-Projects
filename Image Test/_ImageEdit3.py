...
for tNum in range(threadnum):
    ...
    globals()["workQueue" + str(threadID)] = []
    thread = myThread(threadID, "Thread-" + str(threadID), "workQueue" + str(threadID))
    thread.start()
...
while tmpx < globals()["im"].width:
    ...
    while tmpy < globals()["im"].height:
        added = False
        while not added:
            #globals()["workQueue" + str(queueid)].put([tmpx,tmpy,tmppix])
            if len(globals()["workQueue" + str(queueid)]) < pixel_buffer_size:
                globals()["workQueue" + str(queueid)].append([tmpx,tmpy,tmppix])
                added = True
            else:
                print("buffer full")
            queueid = queueid + 1
            if queueid > threadnumber:
                queueid = 1
        tmpy = tmpy + 1
